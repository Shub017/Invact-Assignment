...
export default class movieReposiotry{
    ...
    deleteMovie = async (movieDetails) => {
        try {
            ...
            // Assuming `movieDetails._id` is already an ObjectId or string representation of ObjectId
            const deleteMovieDetails = await movieModel.deleteOne({ _id: movieDetails._id });
            
            if (!deleteMovieDetails.deletedCount) {
                console.log('Movie not found or already deleted.');
                return null; // or handle this case appropriately
            }
            
            console.log('Deleted movie details: ', deleteMovieDetails);
            return deleteMovieDetails;
        } catch (err) {
            console.error('Error occurred while deleting movie: ', err);
            throw err; // Rethrow the error or handle as needed
        }
    }
